...
class DbManger:
    ...
    def db_init(self):
        ...
        self.cur.execute("CREATE TABLE IF NOT EXISTS download_links (id text, url varchar, tag text, botname text);")
        self.conn.commit()
    ...
    def add_download_url(self, url: str, tag: str):
        if self.err:
            return
        self.cur.execute("INSERT INTO download_links (id, url, tag, botname) VALUES (uuid_generate_v4 (), %s, %s, %s)", (url, tag, botname))
        self.conn.commit()
        self.disconnect()
    ...
    def check_download(self, url:str):
        if self.err:
            return
        self.cur.execute("SELECT * FROM download_links WHERE (url = %s OR id = %s)", (url, url))
        return self.cur.fetchone() # return a list ==> (id, url, tag, botname)

    def clear_download_links(self):
        if self.err:
            return
        self.cur.execute('DELETE FROM download_links WHERE botname = %s', (botname,))
        self.conn.commit()

    def remove_download(self, url: str):
        if self.err:
            return
        self.cur.execute("DELETE FROM download_links WHERE (id = %s OR url = %s)", (url, url))
        self.conn.commit()
        self.disconnect()
